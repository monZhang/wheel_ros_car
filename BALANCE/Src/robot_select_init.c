...
void Robot_Select(void) {
    ...
    switch (Car_Mode) {
        case Mec_Car:
            Robot_Init(MEC_wheelspacing, MEC_axlespacing, 0, HALL_30F, Hall_13, Mecanum_75);
            break; //麦克纳姆轮小车
        case Omni_Car:
            Robot_Init(0, 0, Omni_Turn_Radiaus_109, HALL_30F, Hall_13, FullDirecion_60);
            break; //全向轮小车
        case Akm_Car:
...
返回  值：无
**************************************************************************/
void Robot_Init(double wheelspacing, float axlespacing, float omni_turn_radiaus, float gearratio, float Accuracy,
                float tyre_diameter) //
{
